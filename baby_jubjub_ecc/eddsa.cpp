...
eddsa<FieldT, HashT>::eddsa(protoboard<FieldT> &pb,
        ...
        gadget<FieldT>(pb, "eddsa") , a(a), d(d), pk_x(pk_x), pk_y(pk_y), b_x(b_x), b_y(b_y), r_x(r_x), r_y(r_y) , message(message), S(S) 
{
    ...
    encode_point_r.reset(new sha256_ethereum(pb, 256, *encode_point_r_input, *encoded_r, "encode R"));
    encode_point_pk.reset(new sha256_ethereum(pb, 256, *encode_point_pk_input, *encoded_pk, "encode pk"));
    encode_points.reset(new sha256_ethereum(pb, 256, *encode_points_input, *encoded_points, "encode pk"));
    hint.reset(new sha256_ethereum(pb, 256, *hint_input, *h, "hint"));
    ...
    jubjub_isOnCurve1.reset( new isOnCurve <FieldT> (pb, pk_x_packed[0], pk_y_packed[0], a, d, "Confirm public key is on the twiseted edwards curve"));
...
void eddsa<FieldT, HashT>::generate_r1cs_constraints()
{
    ...
    this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>({b_y} , {1}, {FieldT("2626589144620713026669568689430873010625803728049924121243784502389097019475")}),
                           FMT("d == 168696", "eddsa"));
    ...
    this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>({lhs_x[255]} , {1}, {rhs_x}),
                           FMT("lhs_x == rhs_x", "eddsa"));
    this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>({lhs_y[255]} , {1}, {rhs_y}),
...
template<typename FieldT, typename HashT>
void eddsa<FieldT, HashT>::generate_r1cs_witness()
{  
