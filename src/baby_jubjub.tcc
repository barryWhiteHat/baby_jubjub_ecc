...
pointMultiplication<FieldT>::pointMultiplication(protoboard<FieldT> &pb,
        ...
        gadget<FieldT>(pb, annotation_prefix) , a(a), d(d) , x(x), y(y), coef(coef), x_ret(x_ret), y_ret(y_ret)
{

    //performs point multiplicaion using double and add method
    this->doub.resize(253);
    this->add.resize(253);

    x_zero.allocate(pb, "x_zero");
    y_zero.allocate(pb, "y_zero");

    x_intermediary.allocate(pb, 253, FMT("annotation_prefix", " x intermediary"));
    y_intermediary.allocate(pb, 253, FMT("annotation_prefix", " y intermediary"));
